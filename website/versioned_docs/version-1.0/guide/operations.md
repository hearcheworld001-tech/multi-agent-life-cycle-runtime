...
GitHub 镜像仓库
   ...
   v
Docusaurus build -> 静态 HTML/CSS/JS -> pages.dev
```

线上地址：[multi-agent-life-cycle-runtime.pages.dev](https://multi-agent-life-cycle-runtime.pages.dev/)。

Cloudflare Pages 发布的是 Docusaurus 生成的 HTML 静态站点。源文件使用 Markdown/MDX 是为了便于维护，不影响浏览器最终接收 HTML。
...
开发服务器支持热更新。编辑 `website/content/docs/**/*.md` 或 `website/docusaurus.config.ts` 后，页面应自动刷新。
...
输出目录为 `website/build`。该目录是生成物，不应手工编辑。
...
1. 新页面已经加入 `website/sidebars.ts` 导航或由现有页面链接；
...
确认目标 Markdown 文件存在，路径大小写一致，并检查 Docusaurus 的文档路由。文档内优先使用相对 Markdown 链接，导航由 `website/sidebars.ts` 管理；版本页由 Docusaurus 自动生成。
...
4. 构建命令和输出目录是否正确；
5. 相同提交能否在 `website/` 中通过 `npm ci` 和 `npm run build`。
